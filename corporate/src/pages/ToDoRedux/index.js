...
//import action
import { addProjectTodoPage, addTodoTodoPage } from "../../slices/thunks";
...
const ToDoList = () => {
  ...
  const todos = useSelector((state) => state.Todo.todosData);
  const projects = useSelector((state) => state.Todo.projectsData);
...
const [todo, setTodo] = useState({
  ...
  const projectValidation = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(addProjectTodoPage(newProjectData));
  ...
  const validation = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      project: Yup.number().required("Please Enter Project Name"),
    ...
    onSubmit: (values) => {
      if (isEdit) {
        ...
        dispatch(addTodoTodoPage(newTodo));
        const filteredTaskListForProject = newTodos.filter(
  ...
  console.log("selected project id", selectedProject ? selectedProject : null);
  ...
  const handleProjectSelect = (id) => {
    ...
    const filterdTasksList = todos.filter((item) => item.project_id === id);
                        ...
                        >
                          <Link
                            className={`nav-link fs-16 ${
                              item.id === selectedProject?.id ? "active" : null
                            }`}
                          >
                            {item.title}{" "}
                          </Link>
          ...
          >
            ...
            <Row className="g-4 mb-3">
              ...
              <Col lg={6}>
                ...
                {validation.touched.status && validation.errors.project ? (
                  <FormFeedback type="invalid">
